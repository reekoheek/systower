...
func runWatch() {
	...
	clockInterval := fs.Duration("clock", 30*time.Second, "clock polling interval")
	cpuInterval := fs.Duration("cpu", 10*time.Second, "cpu polling interval")
	memInterval := fs.Duration("mem", 10*time.Second, "memory polling interval")
